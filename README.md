...
1. **Сразу** (≈ 0.1 сек) отдаёт CC ответ на `initialize` — либо закэшированный с прошлой удачной сессии (`/tmp/mcp-vanessa-init-cache.json`), либо минимальный валидный stub.
2. На `tools/list` отдаёт список инструментов из кэша (`/tmp/mcp-vanessa-tools-cache.json`) — CC видит все ~27 VA-инструментов, как если бы backend был жив.
3. На `tools/call` без живого backend пробует один раз ленивый реконнект; если 1С всё ещё не отвечает — возвращает чёткую JSON-RPC ошибку с инструкцией («запусти 1С через ... и повтори вызов, `/mcp` не нужен»).
...
| `MCP_PROXY_INIT_CACHE` | `/tmp/mcp-vanessa-init-cache.json` | Путь к кэшу последнего успешного ответа `initialize`. Используется при offline-stub. |
| `MCP_PROXY_TOOLS_CACHE` | `/tmp/mcp-vanessa-tools-cache.json` | Путь к кэшу последнего успешного `tools/list`. Используется при offline-stub. |
